...
def submit(request):
    ...
    if DBSession.query(Application).filter_by(username=username).first():
        return error('You can only submit one application, sorry! If you submitted any information that is incorrect or if you\'d like to revise your entry, you can email <strong><a mailto="openhw2012@fedoraproject.org">openhw2012@fedoraproject.org</a></strong>.')
